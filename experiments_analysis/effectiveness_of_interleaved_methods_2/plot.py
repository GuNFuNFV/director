...
if __name__ == "__main__":
    ...
    for i in df1.values[:7]:
        temp_list = eval(i[2])[:5]
        packet_size = packet_size_list[i[0]]
        for j in range(len(temp_list)):
            for k in range(len(temp_list[j])):
                temp_list[j][k] = temp_list[j][k] * (packet_size + 20) / 1000000000
        interleaved_throughput.append(temp_list)
    print(interleaved_throughput)

    rtc_throughput = []
    for i in df2.values[:7]:
        temp_list = eval(i[2])[:5]
        packet_size = packet_size_list[i[0]]
        for j in range(len(temp_list)):
            for k in range(len(temp_list[j])):
    ...
    plt.plot(packet_size_list, representative_interleaved_throughput, label="interleaved", marker="^", linestyle="-")
    plt.plot(packet_size_list, representative_rtc_throughput, label="monolithic", marker="o", linestyle="--")
    ...
    #                  yerr=np.std(temp_interleaved_throughput), fmt="o")
    ...
    plt.tight_layout()
    plt.savefig("interleaved_vs_rtc.pdf")
    plt.show()
